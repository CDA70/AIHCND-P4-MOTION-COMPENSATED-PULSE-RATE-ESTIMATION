...
## Introduction
A core feature that many users expect from their wearable devices is pulse rate estimation. Continuous pulse rate estimation can be informative for many aspects of a wearer's health. Pulse rate during exercise can be a measure of workout intensity and resting heart rate is sometimes used as an overall measure of cardiovascular fitness. This project provides a pulse rate estimation algorithm for a wrist-wearable device. The information in the [section below](#physiological-mechanics-of-pulse-rate-estimation) explains the design of the algorithm in morte detail. The algorithm is conform to the given [specifications](#algorithm-specifications).
